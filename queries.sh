...
echo "$($PSQL "SELECT DISTINCT(name) FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE winner_id = team_id ORDER BY name")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE name LIKE 'Co%' UNION SELECT DISTINCT(name) FROM games INNER JOIN teams ON games.opponent_id = teams.team_id WHERE name LIKE 'Co%' ORDER BY name")"
